			...
			<div class="well well-small">
				<ul class="nav nav-list">
					...
					<c:if test="${ empty TotalPriceCart }">
						<li><a class="totalInCart" href="gio-hang"><strong>Đã
								mua <span class="badge badge-warning pull-right"
								style="line-height: 18px;"><fmt:formatNumber type="number" groupingUsed="true" value="0" />	 ₫</span>
						</strong></a></li>
					</c:if>
	...
	-->
		<div class="well well-small">
			...
			<div class="row-fluid">
				...
				<c:if test="${ high_products.size() > 0 }">
					<ul class="thumbnails">
					
					<c:forEach var="item" items="${ high_products }" varStatus="loop">
						<li class="span4">
							<div class="thumbnail">
						...
						<c:if test="${ (loop.index + 1) % 3 == 0 || (loop.index + 1)  == high_products.size() }">
							</ul>
							<c:if test="${ (loop.index + 1) < high_products.size() }">
								<ul class="thumbnails">
							</c:if>
						</c:if>
